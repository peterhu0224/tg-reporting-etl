...
from report_config import trans_summary_1m_config


class trans_summary_1m(ProcessStep):
    @classmethod
    def process(cls, data, utils):
        ...
        # 取1h task_list裡屬於trans_summary_1m自己該做的任務
        assignee = 'trans_summary_1m'
        sub_data = data[data['assignee'] == assignee]

        task_conn = db_utils.get_task_db_maria_conn()
        report_conn = getattr(db_utils, trans_summary_1m_config['target_conn_report'])()
    ...
    def delete_before_insert(single_task, conn):
        ...
        delete_sql = f"""
                        ...
                        WHERE summary_month = '{int(single_task['gte_time'].strftime("%Y%m"))}'
                          {platform_filter}
                          {site_code_filter}
                    """
