...
from report_config import trans_summary_1h_config


class trans_summary_1h(ProcessStep):
    @classmethod
    def process(cls, data, utils):
        ...
        # 取1h task_list裡屬於trans_summary_1h自己該做的任務
        assignee = 'trans_summary_1h'
        sub_data = data[data['assignee'] == assignee]

        task_conn = db_utils.get_task_db_maria_conn()
        report_conn = getattr(db_utils, trans_summary_1h_config['target_conn_report'])()
    ...
    def aggregation(cls, single_task, r_conn):
        target_table = 'trans_summary_1h'

        # aggregation交由DB處理
        report = cls.get_trans_report_1h(single_task, r_conn)
    ...
    @staticmethod
    def get_trans_report_1h(single_task, r_conn):
        source_table = 'trans_summary_5min'
