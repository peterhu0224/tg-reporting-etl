...
class risk_ctrl_game_1d(ProcessStep):
    @classmethod
    def process(cls, data, utils):
        ...
        # 取1d_task_list裡屬於risk_control_game_1d自己該做的任務
    ...
    def aggregation(cls, single_task, r_conn, utils):
        target_table = 'risk_ctrl_game_1d'
        ...
        risk_report = meta_df.merge(sigma_df, how='left', left_on=['platform', 'site_code', 'game_code', 'country'],
                                    right_on=['platform', 'site_code', 'game_code', 'country'])
    ...
    def get_metadata(single_task, r_conn):
        ...
        current_sum_sql = f"""
                                SELECT platform, site_code, game_code, country,
                                 ...
                                 WHERE 1=1 
                                   AND summary_date = '{int(single_task['gte_time'].strftime('%Y%m%d'))}'
                                       ...
                                       {site_code_filter}
                                       {game_code_filter}
                                  GROUP BY `platform`, `site_code`, `game_code`, `country`
                           """
        ...
        p_wma_sql = f"""
                            SELECT platform, site_code, game_code, country,
                             ...
                             FROM(
                            SELECT summary_date, platform, site_code, game_code, country, 
                             ...
                             GROUP BY summary_date, `platform`, `site_code`, `game_code`, `country`
                              ) AS a
                             GROUP BY `platform`, `site_code`, `game_code`, `country`;
                       """
        ...
        # 組合報表
        meta_df = current_df.merge(p_wma_df, how='left', left_on=['platform', 'site_code', 'game_code', 'country'],
                                   right_on=['platform', 'site_code', 'game_code', 'country'])
        # 植入報表時間
    ...
    def get_sigma_data(meta_df):
        ...
        """
        b_wma_floor = cfg['default_value_game']['def_b_wma']
        b_wmasd_floor = cfg['default_value_game']['def_b_wmasd']
        ...
        輸贏值轉換 (值小於預設無論正負直接取代)
        """
        p_wma_floor = cfg['default_value_game']['def_p_wma']
        p_wmasd_floor = cfg['default_value_game']['def_p_wmasd']
        ...
        # 只取要的欄位
        sigma_df = tdf[
            ['platform', 'site_code', 'game_code', 'country', 'b_sigma', 'p_sigma', 'risk_score']].copy()
