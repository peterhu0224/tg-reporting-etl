...
from report_config import player_summary_1d_config
from report_config import player_summary_rtp_config


class player_summary_1d(ProcessStep):
    @classmethod
    def process(cls, data, utils):
        ...
        report_conn = getattr(db_utils, player_summary_1d_config['target_conn'])()
    ...
    def aggregation(cls, single_task, r_conn, utils):
        target_table = 'player_summary_1d'

        # aggregation交由DB處理
        report = cls.get_player_profit_report_1d(single_task, r_conn, utils)
    ...
    @staticmethod
    def get_player_profit_report_1d(single_task, r_conn, utils):
        source_table = 'player_summary_1h'
        ...
        report_1d = retry_call(pd.read_sql, fargs=[sql, r_conn], tries=10, delay=2,
                               logger=utils['logger'])

        # rtp或wl可能發生除以0的問題
        report_1d = report_1d.replace([np.inf, -np.inf], np.nan).fillna(0)
