...
from report_config import player_summary_1m_config
from report_config import player_summary_rtp_config

class player_summary_1m(ProcessStep):
    @classmethod
    def process(cls, data, utils):
        ...
        # 取1m task_list裡屬於player_summary_1m自己該做的任務
        assignee = 'player_summary_1m'
        sub_data = data[data['assignee'] == assignee]

        task_conn = db_utils.get_task_db_maria_conn()
        report_conn = getattr(db_utils, player_summary_1m_config['target_conn'])()
        ...
        for i, row in sub_data.iterrows():
            ...
            # 跨月後才更新done=1，若當前done=0就會一直重跑(實現update效果)
    ...
    def aggregation(cls, single_task, r_conn, utils):
        target_table = 'player_summary_1m'

        # aggregation 交由DB處理
        report = cls.get_player_profit_report_1m(single_task, r_conn, utils)
    ...
    @staticmethod
    def get_player_profit_report_1m(single_task, r_conn, utils):
        source_table = 'player_summary_1d'
        ...
        # report_1m = pd.read_sql(sql, r_conn)
        report_1m = retry_call(pd.read_sql, fargs=[sql, r_conn], tries=10, delay=2,
                               logger=utils['logger'])
        # rtp或wl可能發生除以0的問題
        report_1m = report_1m.replace([np.inf, -np.inf], np.nan).fillna(0)

        # 組時間格式
        report_1m['summary_month'] = int(single_task['gte_time'].strftime("%Y%m"))
    ...
    def delete_before_insert(single_task, conn):
        ...
        delete_sql = f"""
                        DELETE FROM {target_table}
                         WHERE summary_month = '{str(int(single_task['gte_time'].strftime("%Y%m")))}'
                           {platform_filter}
                           {site_code_filter}
                           {game_code_filter}
                     """
